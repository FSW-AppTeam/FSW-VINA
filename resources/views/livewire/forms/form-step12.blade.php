        ...
        <div class="form-group student-list col border-end d-flex justify-content-center align-items-center row">
            ...
            @if (empty($startFriend))
                <div class="notification alert alert-danger text-center">
                    <p>Je hebt geen vrienden opgeven!</p>
                </div>
...
{{--                        <h1>jaaazzz  {{$key}}</h1>--}}
                        <div class="justify-content-center row student-block-v2 selected-friends-row-v4 question-step-friends  @if($key !== 0) qv2 @endif" id="selected-row-{{$key}}" style="z-index: {{$rowZindex - $key}}">

                            @else
{{--                                <h1>neee  {{$key}} --- {{ count($friendsList) }}</h1>--}}

                                <div class="justify-content-center row student-block-v2 selected-friends-row-v4 @if(count($friendsList) === 1) v2 @endif @if($key !== 0) selected-friends-row-v3 @endif" id="selected-row-{{$key}}" style="z-index: 0">
                                    @endif
                            ...
                            <div class="container">
                                <div class="row">
                                    @foreach ($this->students as $key => $student)
                                        <livewire:student-fade-component wire:key="students-fade-selected-v2{{ $student['id'] }}" :id="$student['id']" :name="$student['name']" :selected-friends-ids="$selectedFriendsIds"/>
                                    @endforeach
                                </div>
                            </div>
                        </div>
