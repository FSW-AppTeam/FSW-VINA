<div class="h-100">
    ...
    <!-- Edit Modal -->
    @include('livewire.role.components.edit')
    <!-- Edit Category Bulk Modal -->
    @include('livewire.role.components.edit-bulk')
    <!-- Create Modal -->
    @include('livewire.role.components.create')
    <!-- Delete Confirmation Modal -->
    @include('livewire.role.components.delete')
    <!-- View Modal -->
    @include('livewire.role.components.view')


    <button data-bs-toggle="modal" data-bs-target="#createModal"
            class="btn btn-outline-success btn-outline-md mb-2">Create New Role
    </button>
    ...
    <div class="row">
        ...
        <div class="col-md-3">
            ...
            <select wire:model.live="orderBy" id="orderBy" class="form-select">
                <option value="role_id">Role_id</option><option value="name">Name</option><option value="created_at">Created_at</option><option value="updated_at">Updated_at</option>
            </select>
        </div>
    ...
    <div class="table-responsive">
        <table class="table table-hover table-sm">
            ...
            <tbody>
            @forelse($paginatedRoles as $model)
                <tr>
                    <td>
                        <input wire:model.live="selectedRoles" id="{{$model->id}}" value="{{$model->id}}" type="checkbox">
                    </td>
                    <td>{{$model->name}}</td>
                    <td>
                        <a class="btn btn-outline-info btn-sm"
                           href="#" wire:click.stop.prevent="redirectToDetail('roledetails', {{ $model->id }})" >
                            Details
                        </a>
